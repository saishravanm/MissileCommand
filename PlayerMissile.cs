...
namespace MissileCommand
{
    class PlayerMissile
    {
        ...
        public int xT;
        public int yT;
        public PlayerMissile(int xtarget, int ytarget, int screenwidth, Rectangle r)
        {
            //stuff for exploding
            exploded = false;
            explosionspeed = 1;
        ...
        public Boolean circleintersects(EnemyMissle input)
        {
            Boolean output = false;
            int radius = drect.Width / 2;
            double distance1 = Math.Sqrt(Math.Pow(input.drect.X - x, 2) + Math.Pow(input.drect.Y - y, 2));
            double distance2 = Math.Sqrt(Math.Pow(input.drect.X + input.drect.Width - x, 2) + Math.Pow(input.drect.Y + input.drect.Height - y, 2));
            output = distance1 <= radius || distance2 <= radius;
            if (output) { startexploding(); }
            return output;
        }
