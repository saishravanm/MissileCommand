            ...
            switch (gameState)
            {
                ...
                case GameState.GameplayScreen:
                    ...
                    if (newMouse.LeftButton == ButtonState.Pressed && oldMouse.LeftButton == ButtonState.Released && s != null && s.missileIndex < 10)
                    {
                        ...
                        {
                            s.addMissile(new PlayerMissile(plane.rect.X + plane.rect.Width / 2, plane.rect.Y + plane.rect.Height / 2, GraphicsDevice.Viewport.Width, new Rectangle(s.rect.X + s.rect.Width / 2, s.rect.Y - 10, 20, 20), rocket, explos));
                            s.missileIndex++;
                            s.activated = true;
                        }
                        if(s.missileIndex == 10)
